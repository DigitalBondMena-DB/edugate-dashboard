...
@php $pageTitle = 'تعديل تخصص'; @endphp
...
@section('content')
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card-body">
                        <form action="{{ route('majors.update', $row->id) }}" method="POST" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            {{ method_field('PUT') }}
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="facultyUniversities[]" class="form-control" style="height:334px;" multiple>
                                            @foreach($facultyUniversities as $facultyUniversity)
                                                <option value="{{ $facultyUniversity->id }}" {{ in_array($facultyUniversity->id, $selectedFacultyUniversities) ? 'selected' : '' }}>{{ $facultyUniversity->faculty_name }} {{ $facultyUniversity->university_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <button type="submit" class="btn btn-primary pull-right">تعديل الكليه</button>
