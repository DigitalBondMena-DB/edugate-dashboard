              ...
              <tbody>
                @foreach($rows as $feedback)
                  <tr>
                    ...
                    <td class="text-start text-wrap-cell">{{ strlen($feedback->name) > 20 ? substr($feedback->name, 0, 20) . '...' : $feedback->name  }}</td>

                    <td class="text-start text-wrap-cell">
                      @if($feedback->email)
                        <a href="mailto:{{ $feedback->email }}" class="link-dark text-decoration-underline">
                    ...
                    <td class="text-start text-wrap-cell">
                      @if($feedback->phone)
                        <a href="tel:{{ preg_replace('/\s+/', '', $feedback->phone) }}" class="link-dark">
                          {{ $feedback->phone }}
                        </a>
                      @else
                        <span class="text-muted">—</span>
                    ...
                    <td class="text-center">
                      <span class="badge rounded-pill req-type-badge">{{ $feedback->request_type }}</span>
                    </td>
...
<style>
  ...
  .actions-cell{ min-width: 90px; white-space: nowrap; }
  .req-type-badge{
