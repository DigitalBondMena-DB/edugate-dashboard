          ...
          <div class="col-md-3">
            <select name="category_id" id="category_id" class="form-control">
              ...
              @foreach($categories as $category)
                <option value="{{ $category->id }}" {{ $category->id == (request()->category_id ?? null) ? 'selected' : '' }}>
                  {{ $category->ar_title }}
                </option>
              @endforeach
            </select>
        ...
        <div class="table-responsive">
          @if(count($rows))
            <table class="table table-hover align-middle mb-0 fixed-table clean-head">
              ...
              <colgroup>
                <col style="width:8%">   {{-- # --}}
                <col style="width:37%">  {{-- Title AR --}}
                <col style="width: 37%">  {{-- Title EN --}}
                    ...
                    <td class="text-center actions-cell">
                      <div class="d-flex justify-content-center gap-2 flex-wrap">
                        <a href="{{ route('articleSubCategory.edit', $subCategory->id) }}"
                           class="btn btn-outline-primary btn-sm" title="Edit Sub Category">✏️</a>
          ...
          @else
            <div class="text-center py-5">
              ...
              <p class="text-muted mb-4">Start creating beautiful sub categories for your website</p>
