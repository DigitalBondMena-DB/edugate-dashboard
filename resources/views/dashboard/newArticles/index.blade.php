      ...
      <form method="GET" action="{{ route('newArticle.index') }}" class="px-3 py-3 border-bottom">
        <div class="row g-2 align-items-center">
          ...
          <div class="col-md-4">
            <select name="sub_category_id" class="form-control">
              ...
              @foreach($subCategories as $sc)
                <option value="{{ $sc->id }}"
                  @selected((string)($subId ?? request('sub_category_id')) === (string)$sc->id)>
                  {{ $sc->ar_title }} @if($sc->en_title) - {{ $sc->en_title }} @endif
                </option>
              @endforeach
            </select>
          </div>
          ...
          <div class="col-md-2 d-flex gap-2">
            ...
            <a href="{{ route('newArticle.index') }}" class="btn btn-light w-100"><i class="fas fa-sync"></i></a>
          </div>
        </div>
      </form>
        ...
        <div class="table-responsive">
          @if(count($rows))
            <table class="table table-hover align-middle mb-0 fixed-table clean-head">
              ...
              <colgroup>
                <col style="width:8%">   {{-- # --}}
                <col style="width:26%">  {{-- Title AR --}}
                <col style="width:26%">  {{-- Title EN --}}
              ...
              <tbody>
                @foreach($rows as $article)
                  <tr>
                    ...
                    <td class="text-start text-wrap-cell">{{ ($article->ar_title > 30 ? "...".Str::substr($article->ar_title, 0, 30) : $article->ar_title) ?: '-' }}</td>
                    <td class="text-start text-wrap-cell">{{ ($article->en_title > 30 ? Str::substr($article->en_title, 0, 30) . "..." : $article->en_title) ?: '-' }}</td>
                    <td class="text-center">
                      {{-- @dd($article->main_image) --}}
                      <img
                        src="{{ asset('newArticle/' . $article->main_image) }}"
                        alt="Article Image"
                          ...
                          <div class="modal-content">
                            ...
                            <div class="modal-body text-center p-0">
                              <img src="{{ asset('newArticle/' . $article->main_image) }}"
                                   class="img-fluid rounded" style="max-height:70vh;object-fit:contain" alt="">
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <td class="text-center actions-cell">
                      <div class="d-flex justify-content-center gap-2 flex-wrap">
                        ...
                        @if($article->status == 1)
                          <form action="{{ route('newArticle.toggleStatus', $article->id) }}" method="POST" class="d-inline">
                            @csrf @method('PATCH')
          ...
          @else
            <div class="text-center py-5">
              ...
              <a href="{{ route('newArticle.create') }}" class="btn btn-primary btn-round shadow-sm px-4">
...
<style>
  /* تثبيت الأبعاد ومنع السحب للأفق */
  ...
  /* مساحة مناسبة لعمود الأكشن */
  .actions-cell{ min-width: 160px; white-space: nowrap; }

  /* هوفر بسيط للصورة */
  .slider-thumbnail{ transition: transform .25s ease, box-shadow .25s ease; }
  .slider-thumbnail:hover{ transform: scale(1.05); box-shadow: 0 6px 16px rgba(0,0,0,.22); }
